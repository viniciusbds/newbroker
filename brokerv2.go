...
func CreateAndManageDemand(controllerName string, nodePort int, frequency int, numberOfReplicas int, expectedRuntime int, wg *sync.WaitGroup) {
	...
	duration :=  time.Duration(expectedRuntime) * time.Second
	...
	for res := range attacker.Attack(targeter, rate, duration, fmt.Sprintf("app %s on port %d", controllerName, nodePort)) {
		metrics.Add(res)
	}
	metrics.Close()
